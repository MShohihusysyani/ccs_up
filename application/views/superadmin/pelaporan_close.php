                                        ...
                                        foreach ($datapelaporan as $dp) : ?>
                                            <tr class="detail-row" data-href="<?= base_url() ?>superadmin/detail_close/<?= $dp['id_pelaporan']; ?>">
                                                ...
                                                <td style="display: flex; gap: 10px; justify-content: flex-end;">
                                                    <a class="btn btn-sm btn-info finish-action" href="<?= base_url() ?>superadmin/finish_pelaporan/<?= $dp['id_pelaporan']; ?>"><i class="material-icons">launch</i> <span class="icon-name"></span></a>
                                                    <?php $this->session->set_userdata('referred_from', current_url()); ?>
                                                    <div class="btn btn-sm btn-warning reject-action">
...
<script>
    $(document).ready(function() {
        ...
        $('body').on('click', '.detail-row', function(e) {
            ...
            // Periksa apakah klik terjadi di dalam elemen dengan pada aksi
            if ($(e.target).closest('.finish-action, .reject-action, a').length > 0) {
                return;
            }
